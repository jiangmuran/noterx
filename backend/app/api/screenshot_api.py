...
from app.agents.base_agent import _get_client, _is_mimo_openai_compat, _should_retry_openai_without_json_format
...
重要：
- 不管截图类型是什么，只要能看到标题或正文就必须提取！不要因为类型是 cover 就不提取标题。
- 如果这是正文长图或详情页，请尽可能连续、完整提取可见正文，不要只输出摘要。
...
def _should_force_ocr(slot_type: str, title_text: str, content_text: str) -> bool:
    """正文截图或长图如果抽取内容过短，强制再跑一次 OCR。"""
    if not title_text or not content_text:
        return True
    if slot_type in {"content", "other"} and len(content_text) < 120:
        return True
    return False


def _parse_json_best_effort(raw: str) -> dict:
    """容错解析模型返回，兼容代码块和前后附带说明文本。"""
    clean = (raw or "").strip()
    if clean.startswith("```"):
...
async def _vision_call(
    ...
    *,
    ...
    for use_json_mode in (True, False):
        try:
            resp = await client.chat.completions.create(**_build_kwargs(use_json_mode))
    ...
    if last_error:
        logger.warning("quick-recognize parse/call failed: %s", last_error)
    return {"raw_text": last_raw, "error": "JSON解析失败"}
...
async def quick_recognize(
    ...
    try:
        ...
        if _should_force_ocr(slot_type, title_text, content_text):
            try:
                from app.analysis.ocr_processor import OCRProcessor
                ocr = OCRProcessor()
                # OCR 补充优先用原图，避免压缩后小字丢失。
                ocr_result = await ocr.extract_text(
                    image_bytes_raw, client, max_tokens_override=ocr_cap
                )
                ...
                if ocr_title and (not title_text or len(ocr_title) > len(title_text)):
                    result["title"] = ocr_title
                if ocr_content and (not content_text or len(ocr_content) > len(content_text)):
